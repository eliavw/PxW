...
```bash
git remote add origin git@github.com:eliavw/PxW.git
git remote -v
...
```bash
git remote add origin git@github.com:eliavw/PxW.git; git remote -v; git push origin master
```
...
```bash
conda env create -f dependencies-deploy.yaml -n PxW
```
...
```bash
conda activate PxW
conda env update -n PxW -f dependencies-develop.yaml
```
...
```bash
conda activate PxW
python -m ipykernel install --user --name PxW --display-name "PxW"
```
...
1. Go to the [Travis](https://travis-ci.com/eliavw/PxW) page of this repo.
2. See if it ran.
...
and your site should be online at; [https://eliavw.github.io/PxW/](https://eliavw.github.io/PxW/). 
...
> A small wrapper to call the Weka CLI from python (notebooks), cf. https://eliavw.github.io/PxW
